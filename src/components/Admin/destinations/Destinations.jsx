  ...
  const fetchDestinations = async () => {
    try {
      ...
      if (response.data.success) {
        setDestinations(response.data.finddestinations);
                    ...
                    destinations.map((dest, index) => (
                      <tr key={dest._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap flex gap-2">
                          {dest.selectedImages && dest.selectedImages.length > 0 ? (
                            dest.selectedImages.map((img, idx) => (
                              <img
                                key={idx}
                                src={img}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <button
                            onClick={() => navigate('/admin/editdestination', { state: dest })}
